<template>
    <div v-click-away="onClickAway">
        ...
        <input type="text" :name="name" :id="name" v-model="displayValue" readonly @click="handleInputClick"
        ...
        <div v-show="isSelectorOpen" class="mt-2">
            <DateSelector v-if="view === 'days'" :year="year" :month="month" @switch-view="switchView" :disabledDates="disabledDates"
                          @update-year="updateYear" @update-month="updateMonth" @update-date="updateDate" :allowMultipleDates="allowMultipleDates"
            :format="format" v-on:input="$emit('update:modelValue', $event.target.value)" v-bind:value="modelValue"/>
            <MonthSelector v-else-if="view === 'months'" :year="year" :month="month" :month-year="monthYear" @switch-view="switchView"
                           @update-year="updateYear"  @update-month="updateMonth"/>
        </div>
    ...
    mounted() {
        if (this.modelValue) {
            ...
            if (typeof this.modelValue === 'string') {
                ...
                date = DateTime.fromFormat(this.modelValue[this.modelValue.length - 1], this.format);
            }
            this.year = date.year;
