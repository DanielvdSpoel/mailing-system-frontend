<template>
    <div class="bg-white rounded-md text-center lg:col-start-8 lg:col-end-13 lg:row-start-1 xl:col-start-9">
        <div class="flex items-center text-gray-900 px-4 pt-4">
            ...
            <button class="flex-auto font-semibold cursor-pointer hover:bg-gray-50 py-2 px-4"
                    @click="$emit('switchView', 'months')">{{ capitalize(translatedMonth) }} {{ year }}
            </button>
        ...
        <div
            ...
            <button v-for="(day, dayIdx) in dates" :key="day.date" type="button" @click="handleClick(day)"
                    :class="['py-1.5 hover:bg-gray-100 focus:z-10',  dayInCurrentMonth(day) ? (isDateDisabled(day) ? 'bg-gray-100 cursor-not-allowed' : 'bg-white') : 'bg-gray-50', (isSelected(day) || dayIsToday(day)) && 'font-semibold', isSelected(day) && 'text-white', !isSelected(day) && dayInCurrentMonth(day) && !dayIsToday(day) && 'text-gray-900', !isSelected(day) && !dayInCurrentMonth(day) && !dayIsToday(day) && 'text-gray-400', dayIsToday(day) && !isSelected(day) && 'text-indigo-600',
                     dayIdx === 0 && 'rounded-tl-lg', dayIdx === 6 && 'rounded-tr-lg', dayIdx === dates.length - 7 && 'rounded-bl-lg', dayIdx === dates.length - 1 && 'rounded-br-lg']">
                <time :datetime="day.toISODate()"
                      :class="['mx-auto flex h-7 w-7 items-center justify-center rounded-full', isSelected(day) && dayIsToday(day) && 'bg-indigo-600', isSelected(day) && !dayIsToday(day) && 'bg-gray-900']">
                    {{ day.day }}
                </time>
            </button>
        </div>
...
export default {
    ...
    methods: {
        ...
        handleClick(day) {
            ...
            if (this.allowMultipleDates) {
                if (this.modelValue.includes(formattedDate)) {
                    this.$emit('updateDate', this.modelValue.filter(date => date !== formattedDate));
                } else {
        ...
        generateDates() {
            ...
            calculationDate = calculationDate.set({month: this.month, year: this.year});

            //Go to the starting point
            const firstDayOfMonth = calculationDate.startOf('month');

            //Add all the days of past month
            calculationDate = firstDayOfMonth.minus({days: firstDayOfMonth.weekday - 1});
